  ...
  export default {
    ...
    created () {
      ...
      if (!isWeixin) {
        ...
        if(!tool.getCookie('ltUserInfo') && this.merchantId === '100036'){
    ...
    methods: {
      ...
      getCode(){
        ...
        if(this.pengding){
          ...
          core.getLoginCodeForWofen({mobile: this.phone,merchantId: this.merchantId}).then(res => {
            //console.log(res)
      ...
      getActiveWeixin () {
        core.activeWeixinForWofen({merchantId: this.merchantId, referer: this.referer}).then(res => {
          if(res.code && '00' == res.code){
            window.location.href = res.result
          } else {
            this.$toastBox.showToastBox("网络错误")
